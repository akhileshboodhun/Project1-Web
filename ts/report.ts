...
var equipmentEmployeeList : Array<EquipmentEmployee> =[
 ...
 var filteredList : Array<Employee> = employeeList;

 var searchBy = document.querySelector('#SearchBy') as HTMLSelectElement;
 searchBy.onchange = function(event) {
    var searchInput = document.querySelector('input[name="SearchValue"]') as HTMLInputElement;
    searchInput.value ='';
 ...
 function Search(obj){
    ...
    switch(selectedIndex){
        case 1:
        break;
        case 2: filteredList = employeeList.filter(emp => emp.FirstName.toLowerCase().includes(searchValue) || emp.LastName.toLowerCase().includes(searchValue));
        break;
        case 3: filteredList = employeeList.filter(emp => emp.DateOfBirth.toLowerCase().includes(searchValue));
        break;
    }
